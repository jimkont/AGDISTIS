...
public class Evaluator {
    ...
    public void fmeasure() {
        ...
        for (Document document : corpus) {
            try {
                ...
                if (0 < document.getProperty(DocumentText.class).getText().length()) {
                    ...
                    for (NamedEntityInText namedEntity : namedEntities) {
                        if (namedEntity.getLength() > 2) {
                            ...
                            if (correctVotingURL.equals(disambiguatedURL)) {
                            ...
                            } else if (correctVotingURL.startsWith("http://dbpedia.org/resource/")
                                ...
                                log.debug("\t fn: " + correctVotingURL + " -> " + disambiguatedURL);
                                    ...
                                    .startsWith("http://rdflivenews.aksw.org/resource/")) && disambiguatedURL == null) {
                                tn++;
                                log.debug("\t tn: " + correctVotingURL + " -> " + disambiguatedURL);
                            } else if (correctVotingURL.startsWith("http://dbpedia.org/resource/")
                                    && disambiguatedURL.startsWith("http://dbpedia.org/resource/")
                ...
                log.error("Could not process doc: " + documentId, e);
            }
        }
        precision = tp / (tp + fp);
        recall = tp / (tp + fn);
                            ...
                            if (correctVotingURL != null) {
                                if (correctVotingURL.equals(disambiguatedURL)) {
                                    ...
                                    if (correctVotingURL.equals("http://aksw.org/notInWiki")
                                        ...
                                        log.info("\t Closed World Assumption: " + correctVotingURL + " -> "
                                                + disambiguatedURL);
                                    } else {
                                        n++;
    ...
    public void writeFmeasureToFile(BufferedWriter bw) {
        try {
            bw.write("NED maxdepth:\t" + algo.getMaxDepth() + "\tTri.-Th.:\t" + algo.getThreshholdTrigram()
                    + "\tpreci.:\t" + precision + "\trecall:\t" + recall + "\tf1:\t" + fmeasure);
            bw.flush();
        } catch (IOException e) {
            log.error(e.getLocalizedMessage());
        }
